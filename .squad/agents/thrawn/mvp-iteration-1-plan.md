...
**Architecture Reference:** `.squad/agents/thrawn/rust-faster-architecture.md` (Sections 2–4, 6.1, 6.9, 12)
...
|-------|------|-------------------|
| **Mando** | Rust Expert | Core implementation lead — epoch, allocator, hash index |
| **Chirrut** | Systems Programming | Cache-line alignment, atomics, memory layout |
| **Rex** | QA Engineer | CI pipeline, test infrastructure, property tests |
| **Cassian** | Reverse Engineer | Behavioral spec extraction from C++/C# for validation |
| **Grievous** | C++ Expert | Reference consultation — epoch, allocator, hash index |
| **Maul** | Security Expert | Unsafe audit — atomics, pointer arithmetic |
| **Ahsoka** | Performance Guru | Early micro-benchmarks for epoch + hash index |
| **Thrawn** | Lead Architect | Review authority on all PRs |
...
**Who:**
- **Primary:** Rex (QA — CI and project structure)
- **Support:** Mando (Cargo.toml dependencies, edition settings)
- **Review:** Thrawn
...
**Who:**
- **Primary:** Mando (Rust Expert — trait impls, const generics)
- **Support:** Chirrut (alignment validation, repr correctness)
- **Review:** Thrawn
...
**Who:**
- **Primary:** Mando (Rust Expert — thiserror, enum design)
- **Review:** Thrawn, Leia (future API ergonomics)
...
**Who:**
- **Primary:** Mando
- **Support:** Ahsoka (hash quality benchmarking)
- **Review:** Thrawn
...
**Who:**
- **Primary:** Rex (QA — CI/CD)
- **Support:** Mando (toolchain config)
- **Review:** Thrawn
...
**Who:**
- **Primary:** Mando (Rust Expert — trait design, unsafe ser/deser)
- **Support:** Chirrut (memory layout, alignment validation)
- **Consulting:** Grievous (C++ record format reference)
- **Review:** Thrawn, Maul (unsafe audit)
...
**Who:**
- **Primary:** Mando (Rust Expert — atomics, lock-free programming)
- **Support:** Chirrut (cache-line alignment, memory ordering validation)
- **Consulting:** Grievous (C++ epoch system reference), Cassian (behavioral spec)
- **Review:** Thrawn, Maul (unsafe + memory ordering audit)
...
- Formal review (Maul unsafe audit + Chirrut ordering review)
...
**Who:**
- **Primary:** Chirrut (Systems Programming — allocator design)
- **Support:** Mando (Rust API design)
- **Consulting:** Grievous (C++ `MallocFixedPageSize` reference)
- **Review:** Thrawn, Maul (unsafe audit)
...
**Estimated Complexity:** M (Medium)  
**PAW Candidate:** Yes — contains unsafe code (raw pointers, custom allocation), benefits from structured implementation + Maul safety review.
...
**Who:**
- **Primary:** Rex (QA — test design and infrastructure)
- **Support:** Ahsoka (benchmark design), Mando (test impl)
- **Consulting:** Cassian (behavioral reference from C++/C# tests)
- **Review:** Thrawn
...
**Estimated Complexity:** M (Medium)  
**PAW Candidate:** No — test code is straightforward to write once components are stable. Direct implementation by Rex with Ahsoka support.
...
```
Week 1:
  [Rex]     1a: Workspace Setup ──────────────────── (Day 1-2)
  [Rex]     1e: CI Skeleton ──────────────────────── (Day 2-3, after 1a)
  [Mando]   1b: Core Address & Newtypes ──────────── (Day 2-5, after 1a)
  [Mando]   1c: Status & Error Types ─────────────── (Day 2-3, parallel with 1b)

Week 2:
  [Mando]   1d: Hash Utility Traits ──────────────── (Day 1-2, after 1b)
  [Mando]   1f: Record Format & Key/Value Traits ─── (Day 2-5, after 1b+1c) [PAW]
  [Chirrut] 1h: Memory Allocator ─────────────────── (Day 1-5, after 1a) [PAW]

Week 3:
  [Mando]   1g: Epoch System ─────────────────────── (Full week, after 1b) [PAW]
  [Chirrut] 1h: Memory Allocator (cont.) ─────────── (Wrap up + review)

Week 4:
  [Mando]   1g: Epoch System (cont.) ─────────────── (Testing + hardening)
  [Rex]     1i: Foundation Tests & Benchmarks ─────── (Full week, after all)
  [Maul]    Unsafe audit of 1f, 1g, 1h ───────────── (Parallel with 1i)
  [Thrawn]  Architecture review of all components ─── (Continuous)
```
...
**Who:**
- **Primary:** Mando
- **Support:** Chirrut (bit manipulation validation)
- **Review:** Thrawn
...
**Who:**
- **Primary:** Mando
- **Support:** Chirrut (alignment verification, atomic ordering)
- **Review:** Thrawn
...
**Who:**
- **Primary:** Mando (Rust Expert — lock-free algorithms)
- **Support:** Chirrut (memory ordering, cache behavior)
- **Consulting:** Grievous (C++ `InternalHashTable` reference), Cassian (behavioral spec)
- **Review:** Thrawn, Maul (unsafe + ordering audit)
...
- Behavioral spec from C++ reference (Cassian)
- Careful implementation of CAS protocol
- Memory ordering review (Maul)
...
**Who:**
- **Primary:** Mando
- **Support:** Chirrut (ordering), Maul (safety review)
- **Consulting:** Grievous (C++ concurrency protocol), Cassian (behavioral contracts)
- **Review:** Thrawn
...
**Who:**
- **Primary:** Rex (QA — test design)
- **Support:** Ahsoka (benchmark design), Jyn (concurrent test patterns)
- **Consulting:** Cassian (behavioral reference: what does C++ test?)
- **Review:** Thrawn
...
**Estimated Complexity:** M (Medium)  
**PAW Candidate:** No — test code, direct implementation by Rex with support.
...
Week 5 (overlaps Phase 1 Week 4):
  [Mando]   2a: Hash Bucket Entry Types ──────────── (Day 1-2, after 1b)
  [Chirrut] 2b: Overflow Bucket Pool ─────────────── (Day 1-2, after 1h)
  [Mando]   2c: Hash Bucket Structure ────────────── (Day 3-5, after 2a)

Week 6:
  [Mando]   2d: Hash Table Core ──────────────────── (Full week) [PAW]

Week 7:
  [Mando]   2e: Concurrent Operations ────────────── (Full week) [PAW]
  [Maul]    Unsafe audit of 2c, 2d ────────────────── (Parallel)

Week 8:
  [Rex]     2f: Tests & Benchmarks ────────────────── (Full week)
  [Maul]    Unsafe audit of 2e ────────────────────── (Parallel)
  [Ahsoka]  Performance analysis + optimization ───── (After benchmarks)
  [Thrawn]  Architecture conformance review ────────── (End of week)
```
...
8. ✅ All `unsafe` code has Maul-approved safety documentation
...
|------|-----------|-------|
| Epoch ordering bug | Miri testing + Maul formal review + ThreadSanitizer | Maul, Mando |
| Hash index CAS protocol subtle race | Model-based property test (shadow HashMap comparison) | Rex, Mando |
| Allocator leak or corruption | Drop-counting test + Miri + leak sanitizer | Chirrut, Maul |
| Cache-line false sharing | Verify `CachePadded` with benchmark (compare padded vs unpadded) | Ahsoka |
| C++ behavioral divergence | Cassian extracts expected behavior from C++ tests for comparison | Cassian |

---

*This plan reflects the architecture in `.squad/agents/thrawn/rust-faster-architecture.md` and the directives in `.squad/decisions/inbox/`. All team members should read this document before beginning their assigned work items.*
