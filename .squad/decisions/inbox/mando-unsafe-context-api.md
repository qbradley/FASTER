...
- **FFI (Chirrut):** Will need a C FFI wrapper for `unsafe_context` / batch ops.
- **Async (Kenobi):** Async adapter should support batch mode via UnsafeContext.
- **Testing (Jyn):** Batch API needs fuzz testing for refresh interval edge cases.
- **Safety (Maul):** No new `unsafe` code — all safety is through Rust's type system.
