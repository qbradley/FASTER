...
**Agent:** Tarkin (Database/Storage Expert)
**Date:** 2026-03-07
...
- **Chirrut (Device):** The `Ring` API is designed to plug into the `Device` trait's completion-callback model. A `UringDevice` adapter will wrap `Ring` in a future wave.
- **Maul (Safety):** The `unsafe` blocks in `ring.rs` (3 total: read, write, fsync SQE pushes) all have `// SAFETY:` comments. The buffer-lifetime unsafety is documented but not yet type-enforced.
